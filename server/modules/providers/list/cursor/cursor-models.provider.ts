...
export const parseModelsOutput = (text: string): CursorModelRow[] => {
...
export const buildCursorModelsDefinition = (models: CursorModelRow[]): ProviderModelsDefinition => {
  ...
  const defaultValue = models.find((model) => model.default)?.name
    ...
    ?? options.find((option) => option.value === 'composer-2.5-fast')?.value
    ?? options[0]?.value
